...
-define(AUTH_TESTS,
    ...
    {ok, DeniedR2} =
        httpc:request(
            get,
            {"http://localhost:8081/metrics", [{"Authorization", "Basic cXdlOnF3ZQ=="}]},
            [],
            []
        ),
    ...
    {ok, DeniedR3} =
        httpc:request(
            get,
            {"http://localhost:8081/metrics", [{"Authorization", "Basic abba"}]},
            [],
            []
        ),
    ...
    {ok, DeniedR4} =
        httpc:request(
            get,
            {"http://localhost:8081/metrics", [{"Authorization", "Bearer abba"}]},
            [],
            []
        ),
    ...
    {ok, BasicLPR} =
        httpc:request(
            get,
            {"http://localhost:8081/metrics", [{"Authorization", "Basic cXdlOnF3YQ=="}]},
            [],
            []
        ),
...
prometheus_httpd_standalone(Config) ->
    ...
    ?assertMatch(
        [
            {"content-length", ExpectedHTMLCL},
            {"content-type", ExpectedHTMLCT}
            | _
        ] when
            ExpectedHTMLCL > 0,
        headers(HTMLResponse)
    ),
...
prometheus_httpd_negotiation(_Config) ->
    {ok, TextResponse} =
        httpc:request(
            get, {"http://localhost:8081/metrics", [{"Accept-Encoding", "deflate"}]}, [], []
        ),
...
prometheus_httpd_negotiation_fail(_Config) ->
    {ok, IdentityResponse} =
        httpc:request(get, {"http://localhost:8081/metrics", [{"Accept-Encoding", "qwe"}]}, [], []),
    ...
    {ok, FEResponse} =
        httpc:request(
            get, {"http://localhost:8081/metrics", [{"Accept-Encoding", "qwe, *;q=0"}]}, [], []
        ),
    ...
    {ok, CTResponse} =
        httpc:request(get, {"http://localhost:8081/metrics", [{"Accept", "image/png"}]}, [], []),
